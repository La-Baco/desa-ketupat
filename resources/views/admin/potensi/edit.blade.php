...
@section('title', 'Edit Potensi - Admin Desa Ketupat')
@section('page_title', 'Edit Data Potensi Desa')
...
<div class="max-w-3xl bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-200/80 dark:border-slate-700/60">
    <form method="POST" action="{{ route('admin.potensi.update', $potensi->id) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="name" value="{{ old('name', $potensi->name) }}" required class="w-full px-4 py-3 rounded-2xl bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-emerald-500 outline-none">
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <select name="category" required class="w-full px-4 py-3 rounded-2xl bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-emerald-500 outline-none">
                    @foreach($categories as $cat)
                        <option value="{{ $cat }}" {{ $potensi->category == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="location" value="{{ old('location', $potensi->location) }}" class="w-full px-4 py-3 rounded-2xl bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-emerald-500 outline-none">
            </div>
        </div>
        ...
        <div class="space-y-2">
            ...
            <textarea name="description" rows="6" required class="w-full px-4 py-3 rounded-2xl bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-emerald-500 outline-none">{{ old('description', $potensi->description) }}</textarea>
        </div>

        <div class="space-y-2">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_featured" value="1" {{ $potensi->is_featured ? 'checked' : '' }} class="w-4 h-4 rounded border-slate-400 text-emerald-600 focus:ring-emerald-500">
        ...
        <div class="space-y-2">
            ...
            @if($potensi->image)
                @php
                    $pImg = Str::startsWith($potensi->image, 'images/') ? asset($potensi->image) : asset('storage/' . $potensi->image);
                @endphp
                <img src="{{ $pImg }}" alt="Gambar" class="h-28 rounded-xl object-cover mt-2">
            @endif
        </div>
